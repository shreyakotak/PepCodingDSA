...
public class LevelOrderOfGTZigzag {
  ...
  public static void levelOrderLinewiseZZ(Node node){
    ...
    while(parentstack.size() > 0){
        //1. get+pop
        Node nn = parentstack.pop();
        
        //2. print
        System.out.print(nn.data + " ");
        
        //3. Add children in childstack according to level no
        if(levelno%2 == 0){
            for(Node child:nn.children){
                childstack.push(child);
            }
        }else{
            ArrayList<Node> children = nn.children;
            for(int i=children.size()-1;i>=0;i--){
                childstack.push(children.get(i));
            }
        }
        
        //4. If parentstack is empty then swap childstack with parentstack
        if(parentstack.size() == 0){
            Stack<Node> temp = parentstack;
            parentstack = childstack;
            childstack = temp;
            
            //level change so hit enter and update level no
            System.out.println();
            levelno++;
        }
    }*/
    ...
    while(parentstack.size() > 0){
      ...
      //4. Swap childstack with parentstack
      Stack<Node> temp = parentstack;
      parentstack = childstack;
      childstack = temp;
      
      //5. level change so hit enter and update level no
      System.out.println();
      levelno++;
    }
  }
